/**
 * Honningsvåg World 2.0 destination QA.
 */
...
const dataPath = path.join(
  root,
  "src/data/honningsvag-cruise-schedules.generated.json",
);
if (!existsSync(dataPath)) {
  fail("missing honningsvag-cruise-schedules.generated.json");
  process.exit(1);
}
...
if (data.port !== "honningsvag") {
  fail(`generated port ${data.port}, expected honningsvag`);
} else {
  pass("generated JSON filtered to port === honningsvag");
}
...
if (integrity.total !== expected.total || rows.length !== expected.total) {
  ...
  pass(`total Honningsvåg calls ${expected.total}`);
}
...
const required = [
  ...
  "src/app/honningsvag-port-guide/page.tsx",
  "src/app/one-day-in-honningsvag/page.tsx",
  "src/app/is-north-cape-worth-visiting/page.tsx",
  "src/app/bird-safaris-honningsvag/page.tsx",
  "src/app/north-cape-tours/page.tsx",
  "src/app/king-crab-tours/page.tsx",
  "src/app/best-time-to-visit-honningsvag/page.tsx",
...
const preserved = [
  "src/app/page.tsx",
  "src/app/excursions/north-cape-vip-experience/page.tsx",
  "src/app/excursions/gjesvaer-bird-safari/page.tsx",
  "src/app/excursions/north-cape-king-crab/page.tsx",
  "src/app/excursions/north-cape-sami-camp-viewpoints/page.tsx",
  "src/app/excursions/coastal-treasures-rib-safari/page.tsx",
  "src/app/excursions/private-arctic-fishing-tour/page.tsx",
  "src/app/excursions/taste-of-the-arctic-ebike-tour/page.tsx",
  "src/app/excursions/bruket-nordvagen-arctic-village/page.tsx",
  "src/app/excursions/north-cape-midnight-sun/page.tsx",
  "src/app/excursions/aurora-explorer-northern-lights/page.tsx",
  "src/app/honningsvag-port-guide/page.tsx",
  "src/app/one-day-in-honningsvag/page.tsx",
  "src/app/is-north-cape-worth-visiting/page.tsx",
  "src/app/bird-safaris-honningsvag/page.tsx",
...
pass("no /excursions redirect required; Honningsvåg hub is already /excursions");
...
if (/Return to ship on time|fit comfortably within/i.test(home)) {
  fail("homepage still uses overconfident return/fit guarantees");
} else {
  pass("homepage softens return-to-ship / fit guarantees");
}
if (!/North Cape plateau|plateau day/i.test(home)) {
  fail("homepage missing North Cape decision hierarchy");
} else {
  pass("homepage keeps North Cape as primary decision frame");
}
...
for (const rel of chromeFiles) {
  ...
  if (
    /Lysefjord|Pulpit Rock|Preikestolen|Bryggen|Mostraumen|Nidaros|Bakklandet|Flamsbana|Stegastein|Dalsnibba|Flydalsjuvet|Vøringsfossen|Hardangervidda|Art Nouveau|Atlantic Ocean Road|Mount Varden/.test(
      text,
    )
  ) {
...
if (!config.includes("honningsvagshoreexcursions.com")) {
  fail("canonical domain missing from site-config");
} else {
  pass("canonical domain honningsvagshoreexcursions.com present");
}
...
if (!config.includes("hello@honningsvagshoreexcursions.com")) {
  fail("reserved contact email missing from config");
} else {
  pass("reserved contact email present in config");
}

if (/mailto:hello@honningsvagshoreexcursions\.com/.test(config)) {
...
if (
  /mailto:hello@honningsvagshoreexcursions\.com/.test(footer) ||
...
if (
  ...
  !/mailto:hello@honningsvagshoreexcursions\.com/.test(contactPage)
) {
  pass("contact page gates mailto behind verification");
} else if (/mailto:hello@honningsvagshoreexcursions\.com/.test(contactPage)) {
...
if (monthKeys.length !== expected.months) {
  ...
  pass(`${expected.months} populated Honningsvåg months`);
}
...
if (
  !/northernLightsLauklines[\s\S]*WRONG_LOCATION|Lauklines[\s\S]*WRONG_LOCATION/.test(
    provenance,
  )
) {
  fail("Lauklines aurora not flagged WRONG_LOCATION");
} else {
  pass("Lauklines aurora provenance flagged WRONG_LOCATION");
}

const aurora = readFileSync(
  path.join(root, "src/lib/excursions/aurora-explorer-northern-lights.ts"),
  "utf8",
);
if (/Lauklines|northernLightsLauklines|siteImages\.northernLights\b/.test(aurora)) {
  fail("aurora excursion still references Lauklines / northernLights asset");
} else {
  pass("aurora excursion does not render Lauklines asset");
}
if (!/never guaranteed|not guaranteed|Visibility depends/i.test(aurora)) {
  fail("aurora excursion missing visibility honesty language");
} else {
  pass("aurora excursion includes visibility honesty");
}

const vip = readFileSync(
  path.join(root, "src/lib/excursions/north-cape-vip-experience.ts"),
  "utf8",
);
if (/Timed for typical Honningsvåg port calls/i.test(vip)) {
  fail("North Cape VIP still uses 'timed for typical port calls' claim");
} else {
  pass("North Cape VIP logistics language softened");
}
if (!/hours ashore alone|do not prove|does not prove/i.test(vip)) {
  fail("North Cape VIP missing hard-gate buffer language");
} else {
  pass("North Cape VIP includes hard-gate planning language");
}
if (!/entrance fees/i.test(vip)) {
  fail("North Cape VIP missing entrance-fee notIncluded note");
} else {
  pass("North Cape VIP notes entrance fees may not be included");
}

const midnight = readFileSync(
  path.join(root, "src/lib/excursions/north-cape-midnight-sun.ts"),
  "utf8",
);
if (!/weather-dependent|conditions allow|when skies are clear/i.test(midnight)) {
  fail("midnight sun tour missing visibility honesty");
} else {
  pass("midnight sun tour softens visibility claims");
}

const northCapeTourGuide = readFileSync(
  path.join(root, "src/app/north-cape-tours/page.tsx"),
  "utf8",
);
if (/return scheduling for typical port/i.test(northCapeTourGuide)) {
  fail("north-cape-tours guide still claims typical-port scheduling guarantee");
} else {
  pass("north-cape-tours guide softens typical-port scheduling claim");
}

console.log(
  "\nINFO: sync source = norway-shore-excursions generated JSON, filter port===honningsvag",
);
if (process.exitCode) {
  console.error("\nQA FAILED");
  process.exit(1);
