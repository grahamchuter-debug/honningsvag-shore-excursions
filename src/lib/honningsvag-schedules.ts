import schedulePayload from "@/data/honningsvag-cruise-schedules.generated.json";

export type HonningsvagScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type HonningsvagScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: HonningsvagScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const honningsvagScheduleIntegrity = payload.integrity;
export const honningsvagScheduleMeta = {
  port: payload.port,
...
function toEntry(row: HonningsvagScheduleRow): HonningsvagScheduleEntry {
  return {
...
export function getAllHonningsvagRows(): HonningsvagScheduleRow[] {
  return payload.rows;
}

export function getHonningsvagEntries(): HonningsvagScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getHonningsvagEntriesForMonthKey(
  monthKey: string,
): HonningsvagScheduleEntry[] {
  return payload.rows
...
export function getHonningsvagMonthKeysWithCalls(): string[] {
  return [
    ...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7))),
  ].sort();
}

export function getHonningsvagMonthSummaries() {
...
export const scheduleDisclaimer =
  "Published times come from the Norway Shore Excursions master schedule import. Always confirm arrival, departure and all aboard with your cruise line before finalising plans. Hours ashore alone do not prove that a North Cape plateau day will fit.";
